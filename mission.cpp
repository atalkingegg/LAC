/*
    ...
    this code were derived from "gl-117", by Thomas A. Drexl and2
...
extern bool SkipTargetTourWhileRefueling;
...
if (MyNetworkId %2)
   ...
   if (AirfieldRequested == 1)
      { 
      display ((char *)"ArmPlayerAtRequestedField() SPAWNING AT RED HQ (29)", LOG_MOST);
      fplayer->tl->x = 300; 
      fplayer->tl->z = 5.00;
      } 
   ...
   if (AirfieldRequested == 1)
      { 
      display ((char *)"ArmPlayerAtRequestedField() SPAWNING AT BLUE HQ (28)", LOG_MOST);
      fplayer->tl->x = -400; 
      fplayer->tl->z = 5.00;
      } 
   if (AirfieldRequested == 2)
...
int MissionNetworkBattle01::processtimer (Uint32 dt)
    ...
    if (MumbleTimeRemainingBeforeChannelReset < 1000)
       {
       event_CommsMacro17();
       MumbleTimeRemainingBeforeChannelReset = 10000000;
...
if (fplayer->id != FIGHTER_ME163)
   ...
   if (LandingTimer > 1000 && fplayer->realspeed < 0.01)
      ...
      if (CurrentMissionNumber == MISSION_NETWORKBATTLE03)
         ...
         if (!SkipTargetTourWhileRefueling)
            { 
        ...
        if (fplayer->HistoricPeriod > 1 && AirfieldChosenForLanding == 1)
           ...
           for (i2 = 0; i2 < missiletypes; i2 ++)
               ...
               if (i2 == 0)
                  { 
                  fplayer->missiles [i2] /= 3;
                  display ((char *)"RearmRefuelRepair() diminishing RedTeam late-war bomb load due to heavy HQ airfield damage", LOG_MOST);
                  }
               if (i2 == 5)
                  { 
                  fplayer->missiles [i2] /=3;
                  display ((char *)"RearmRefuelRepair() diminishing RedTeam late-war rocket load due to heavy HQ airfield damage", LOG_MOST);
                  }
               }
           }
        else
